...
A plan can own multiple attempts; the UI derives Active and Partial states from
those attempts while the stored enum keeps terminal scheduling state.

### StudySession

An actual study session, optionally linked many-to-one to a `ScheduledSession`.
Tracks `startedAt`, `endedAt`, `lastHeartbeatAt`, `actualDurationSec`, the
session's title/goal/taxonomy snapshots, `scoreStatus`, `qualityScore`,
`qualityVersion`, the explainable `qualityBreakdown`, and
`autoClosed`/`autoCloseReason`. Legacy counters remain for old records. A partial
unique index allows only one open session (`endedAt IS NULL`) per user.

### SessionActivity

Metadata-only evidence used by Session Quality v2. Stores an activity type,
source identifier, session, owner, and timestamp—never prompts, questions,
answers, summary content, or tutor messages. `(sessionId, type, sourceId)` is
unique so retries cannot inflate a score.
